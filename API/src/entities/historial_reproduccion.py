...
class HistorialReproduccion(Base):
    ...
    Atributos:
        ...
        tiempo_visto: Tiempo visto en minutos
        fecha_visualizacion: Fecha en que se vio la obra
        id_perfil: FK hacia perfil
        id_obra: FK hacia obra
    """
    ...
    tiempo_visto: int = Column(
        Integer,
        nullable=False
    )

    fecha_visualizacion: datetime = Column(
        DateTime,
    ...
    id_perfil: uuid.UUID = Column(
        UUID(as_uuid=True),
        ForeignKey("perfiles.id_perfil"),
        nullable=False
    )

    id_obra: uuid.UUID = Column(
        UUID(as_uuid=True),
        ForeignKey("obras.id_obra"),
        nullable=False
    )
